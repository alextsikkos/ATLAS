...
try {
    ...
    $items = $null
    try { $items = Get-AntiPhishPolicy -ErrorAction Stop } catch { $errors["Get-AntiPhishPolicy"] = $_.Exception.Message }

    $rows = @()
    if ($items) {
        foreach ($p in @($items)) {
            $row = @{ identity = "$($p.Identity)" }
            foreach ($k in @("EnableMailboxIntelligence","EnableMailboxIntelligenceProtection","EnableSpoofIntelligence","PhishThresholdLevel","EnableOrganizationDomainsProtection","EnableTargetedDomainsProtection","EnableTargetedUserProtection","TargetedUserProtectionAction","TargetedDomainProtectionAction","MailboxIntelligenceProtectionAction","EnableFirstContactSafetyTips","EnableSimilarUsersSafetyTips","EnableSimilarDomainsSafetyTips","EnableUnusualCharactersSafetyTips")) {

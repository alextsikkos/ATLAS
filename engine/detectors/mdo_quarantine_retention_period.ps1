...
try {
    ...
    $items = $null
    try { $items = Get-QuarantinePolicy -ErrorAction Stop } catch { $errors["Get-QuarantinePolicy"] = $_.Exception.Message }

    $rows = @()
    if ($items) {
        foreach ($p in @($items)) {
            $row = @{ identity = "$($p.Identity)" }
            foreach ($k in @("QuarantineRetentionDays","EndUserQuarantinePermissions","ESNEnabled")) {

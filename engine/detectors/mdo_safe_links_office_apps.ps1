...
try {
    ...
    $items = $null
    try { $items = Get-SafeLinksPolicy -ErrorAction Stop } catch { $errors["Get-SafeLinksPolicy"] = $_.Exception.Message }

    $rows = @()
    if ($items) {
        foreach ($p in @($items)) {
            $row = @{ identity = "$($p.Identity)" }
            foreach ($k in @("EnableSafeLinksForOffice","TrackClicks","AllowClickThrough","ScanUrls")) {

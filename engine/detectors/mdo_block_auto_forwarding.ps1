...
try {
    $errors = @{}
    $cmdletsUsed = @()
    $cmdletsUsed += "Get-RemoteDomain"
    $items = $null
    try { $items = Get-RemoteDomain -ErrorAction Stop } catch { $errors["Get-RemoteDomain"] = $_.Exception.Message }

    $rows = @()
    if ($items) {
        foreach ($p in @($items)) {
            $row = @{ identity = "$($p.Identity)" }
            foreach ($k in @("AutoForwardEnabled","AllowedOOFType","TNEFEnabled")) {

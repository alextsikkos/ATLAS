...
try {
    ...
    $items = $null
    try { $items = Get-AtpPolicyForO365 -ErrorAction Stop } catch { $errors["Get-AtpPolicyForO365"] = $_.Exception.Message }

    $rows = @()
    if ($items) {
        foreach ($p in @($items)) {
            $row = @{ identity = "$($p.Identity)" }
            foreach ($k in @("EnableATPForSPOTeamsODB","EnableSafeLinksForO365Clients","EnableSafeDocs","EnableATPForSharePoint")) {

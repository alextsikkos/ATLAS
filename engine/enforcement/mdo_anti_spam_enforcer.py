# engine/enforcement/mdo_anti_spam_enforcer.py
...
def _enforce_mdo_anti_spam(**kwargs) -> tuple[str, str, str, dict, int]:
...
try {{
  ...
  if ($before.ruleCount -eq 0) {{
    $baselineRuleName = "ATLAS Baseline AntiSpam"

    # Choose target policy:
    # - never assume default is safe to mutate; prefer a non-default policy if any exist
    $targetPolicy = $null
    $custom = @($before.policies | Where-Object {{ $_.IsDefault -eq $false }} | Select-Object -First 1)
    ...
    if (-not $targetPolicy) {{
      ...
      $targetPolicy = Get-HostedContentFilterPolicy |
      ...
      if (-not $targetPolicy -and $createPolicyError) {{
        $out = [PSCustomObject]@{{
          ...
          reasonDetail = "Failed to create ATLAS AntiSpam policy: $createPolicyError"
          before       = $before
        }}
        $out | ConvertTo-Json -Depth 10
        exit 0
    ...
    if (-not $targetPolicy -or -not $targetPolicy.Identity) {{
      $out = [PSCustomObject]@{{
        ...
        reasonDetail = "No usable HostedContentFilter policy identity available for baseline enforcement."
        before       = $before
      }}
      $out | ConvertTo-Json -Depth 10
      exit 0
    ...
    New-HostedContentFilterRule -Name $baselineRuleName -HostedContentFilterPolicy $targetPolicy.Identity -RecipientDomainIs @("*") -Priority 0 -ErrorAction Stop | Out-Null
  ...
  if ($before.ruleCount -gt 1) {{
    $out = [PSCustomObject]@{{
      ...
      reasonDetail = "Multiple HostedContentFilter rules exist and none are enabled; manual selection required."
      before       = $before
    }}
    $out | ConvertTo-Json -Depth 10
    exit 0
  ...
  try {{
    Set-HostedContentFilterRule -Identity $r.Identity -State Enabled -ErrorAction Stop | Out-Null
  }} catch {{
    try {{
      Set-HostedContentFilterRule -Identity $r.Name -State Enabled -ErrorAction Stop | Out-Null
    }} catch {{
      throw
    }}
  }}
...
"""
    ...
    if code != 0:
        return (
            "ERROR",
            "ENFORCER_ERROR",
            "PowerShell execution failed for MDOAntiSpam",
    ...
    if mode != "enforce":
        if compliant:
            return ("COMPLIANT", "REPORT_ONLY_EVALUATED", "Report-only: anti-spam rule enabled", {"result": result}, 200)
        return ("DRIFTED", "REPORT_ONLY_EVALUATED", "Report-only: no enabled anti-spam rules found", {"result": result}, 200)

    # enforce evaluation
    if compliant:
        return ("COMPLIANT", "ENFORCER_EXECUTED", "Enforcer executed; anti-spam rule enabled/verified", {"result": result}, 200)
    return ("DRIFTED", "ENFORCER_EXECUTED", "Enforcer executed but verification did not show enabled rule", {"result": result}, 200)


# Register
register("MDOAntiSpam", _enforce_mdo_anti_spam)

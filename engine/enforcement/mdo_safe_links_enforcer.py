# engine/enforcement/mdo_safe_links_enforcer.py
...
def _enforce_mdo_safe_links(**kwargs) -> tuple[str, str, str, dict, int]:
...
try {{
  ...
  if ($before.ruleCount -eq 0) {{
    $baselineRuleName = "ATLAS Baseline SafeLinks"

    # Target policy selection:
    # - never mutate built-in protection policies
    # - prefer existing non-built-in if present
    $targetPolicy = $null
    ...
    if (-not $targetPolicy) {{
      ...
      try {{
        # Tier-3 safe baseline: enable Safe Links across Email/Office/Teams.
        New-SafeLinksPolicy -Name $atlasPolicyName `
      ...
      $targetPolicy = Get-SafeLinksPolicy |
      ...
      if (-not $targetPolicy -and $createPolicyError) {{
        $out = [PSCustomObject]@{{
          ...
          reasonDetail = "Failed to create ATLAS Safe Links policy: $createPolicyError"
          before       = $before
        }}
        $out | ConvertTo-Json -Depth 10
        exit 0
    ...
    if (-not $targetPolicy -or -not $targetPolicy.Identity) {{
      $out = [PSCustomObject]@{{
        ...
        reasonDetail = "No usable Safe Links policy identity available for baseline enforcement."
        before       = $before
      }}
      $out | ConvertTo-Json -Depth 10
      exit 0
    ...
    New-SafeLinksRule -Name $baselineRuleName -SafeLinksPolicy $targetPolicy.Identity -RecipientDomainIs @("*") -Priority 0 -ErrorAction Stop | Out-Null
  ...
  if ($before.ruleCount -gt 1) {{
    $out = [PSCustomObject]@{{
      ...
      reasonDetail = "Multiple Safe Links rules exist and none are enabled; manual selection required."
      before       = $before
    }}
    $out | ConvertTo-Json -Depth 10
    exit 0
  ...
  try {{
    Set-SafeLinksRule -Identity $r.Identity -State Enabled -ErrorAction Stop | Out-Null
  }} catch {{
    try {{
      Set-SafeLinksRule -Identity $r.Name -State Enabled -ErrorAction Stop | Out-Null
    }} catch {{
      throw
    }}
  }}
...
"""
    ...
    if code != 0:
        return (
            "ERROR",
            "ENFORCER_ERROR",
            "PowerShell execution failed for MDOSafeLinks",
    ...
    if mode != "enforce":
        if compliant:
            return ("COMPLIANT", "REPORT_ONLY_EVALUATED", "Report-only: safe links rule enabled", {"result": result}, 200)
        return ("DRIFTED", "REPORT_ONLY_EVALUATED", "Report-only: no enabled safe links rules found", {"result": result}, 200)

    if compliant:
        return ("COMPLIANT", "ENFORCER_EXECUTED", "Enforcer executed; safe links rule enabled/verified", {"result": result}, 200)
    return ("DRIFTED", "ENFORCER_EXECUTED", "Enforcer executed but verification did not show enabled rule", {"result": result}, 200)


register("MDOSafeLinks", _enforce_mdo_safe_links)
